...
class dynamic_trajectory_converter(object):
  def __init__(self):
    ...
    if moto_ns: # If the namspace isn't empty, add a '/'
      moto_ns = moto_ns + "/"
    dof_ns = rospy.get_param('~7dof_ns','dx200_7dof')
    if dof_ns: # If the namspace isn't empty, add a '/'
      dof_ns = dof_ns + "/"

    # Subscribe to joint_states from robot/simulator
    rospy.Subscriber('r1_controller/joint_states',
                     JointState, self.r1_joint_state_cb)
    rospy.Subscriber('s1_controller/joint_states',
    ...
    rospy.Subscriber(moto_ns + '/r1_controller/feedback_states',
                     FollowJointTrajectoryFeedback, self.r1_feedback_state_cb)
    rospy.Subscriber(moto_ns + '/s1_controller/feedback_states',
  ...
  def pub_joint_states(self):
    ...
    if (self.r1_joint_states.header.seq != self.s1_joint_states.header.seq):
      ...
      name = ['joint_s','joint_l','joint_u','joint_r','joint_b','joint_t','joint_x']
  ...
  def pub_feedback_states(self):
    ...
    if (self.r1_feedback_states.header.seq != self.s1_feedback_states.header.seq):
      ...
      joint_names = ['joint_s','joint_l','joint_u','joint_r','joint_b','joint_t','joint_x']
      positions = self.r1_feedback_states.actual.positions + (self.s1_feedback_states.actual.positions[0],)
      if (len(self.s1_feedback_states.actual.velocities) < 1) :
        velocities = self.r1_feedback_states.actual.velocities + (0.0,)
      else:
        velocities = self.r1_feedback_states.actual.velocities + (self.s1_feedback_states.actual.velocities[0],)
