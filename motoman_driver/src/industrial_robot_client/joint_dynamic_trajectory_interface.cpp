#include <algorithm>
#include "motoman_driver/industrial_robot_client/joint_dynamic_trajectory_interface.h"
...
{
namespace joint_dynamic_trajectory_interface
{

#define ROS_ERROR_RETURN(rtn,...) do {ROS_ERROR(__VA_ARGS__); return(rtn);} while(0)

bool JointDynamicTrajectoryInterface::init(std::string default_ip, int default_port)
{
  std::string ip;
  int port;
...
bool JointDynamicTrajectoryInterface::init(SmplMsgConnection* connection)
{
...
bool JointDynamicTrajectoryInterface::init(SmplMsgConnection* connection, const std::vector<std::string> &joint_names,
                                    const std::map<std::string, double> &velocity_limits)
{
  ...
  this->srv_stop_motion_ = this->node_.advertiseService("stop_motion", &JointDynamicTrajectoryInterface::stopMotionCB, this);
  this->srv_joint_trajectory_ = this->node_.advertiseService("joint_path_command", &JointDynamicTrajectoryInterface::jointTrajectoryCB, this);
  this->sub_joint_trajectory_ = this->node_.subscribe("joint_path_command", 0, &JointDynamicTrajectoryInterface::jointTrajectoryCB, this);
  this->sub_cur_pos_ = this->node_.subscribe("joint_states", 1, &JointDynamicTrajectoryInterface::jointStateCB, this);

  return true;
}

JointDynamicTrajectoryInterface::~JointDynamicTrajectoryInterface()
{
  trajectoryStop();
  this->sub_joint_trajectory_.shutdown();
}

bool JointDynamicTrajectoryInterface::jointTrajectoryCB(industrial_msgs::CmdJointTrajectory::Request &req,
...
void JointDynamicTrajectoryInterface::jointTrajectoryCB(const trajectory_msgs::JointTrajectoryConstPtr &msg)
{
...
bool JointDynamicTrajectoryInterface::trajectory_to_msgs(const trajectory_msgs::JointTrajectoryConstPtr& traj, std::vector<SimpleMessage>* msgs)
{
  msgs->clear();
...
bool JointDynamicTrajectoryInterface::select(const std::vector<std::string>& ros_joint_names, const ros_JointTrajPt& ros_pt,
...
bool JointDynamicTrajectoryInterface::calc_velocity(const trajectory_msgs::JointTrajectoryPoint& pt, double* rbt_velocity)
{
...
bool JointDynamicTrajectoryInterface::calc_duration(const trajectory_msgs::JointTrajectoryPoint& pt, double* rbt_duration)
{
...
bool JointDynamicTrajectoryInterface::create_message(int seq, const trajectory_msgs::JointTrajectoryPoint &pt, SimpleMessage *msg)
{
...
void JointDynamicTrajectoryInterface::trajectoryStop()
{
...
bool JointDynamicTrajectoryInterface::stopMotionCB(industrial_msgs::StopMotion::Request &req,
...
bool JointDynamicTrajectoryInterface::is_valid(const trajectory_msgs::JointTrajectory &traj)
{
...
void JointDynamicTrajectoryInterface::jointStateCB(const sensor_msgs::JointStateConstPtr &msg)
{
  this->cur_joint_pos_ = *msg;
}

} //joint_dynamic_trajectory_interface
