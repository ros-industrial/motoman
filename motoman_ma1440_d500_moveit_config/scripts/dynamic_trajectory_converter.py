...
class dynamic_trajectory_converter(object):
  def __init__(self):
    ...
    rospy.Subscriber(moto_ns + 'r1_controller/joint_states',
                     JointState, self.r1_joint_state_cb)
    rospy.Subscriber(moto_ns + 's1_controller/joint_states',
  ...
  def pub_feedback_states(self):
    ...
    if (self.r1_feedback_states.header.seq != self.s1_feedback_states.header.seq):
      ...
      # rospy.logwarn(self.s1_feedback_states.actual)

      positions = self.r1_feedback_states.actual.positions + (self.s1_feedback_states.actual.positions[0], 0.0)
      if (len(self.s1_feedback_states.actual.velocities) < 1) :
        velocities = self.r1_feedback_states.actual.velocities + (0.0,)
      else:
        velocities = self.r1_feedback_states.actual.velocities + (self.s1_feedback_states.actual.velocities[0], 0.0)
